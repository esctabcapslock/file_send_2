...
var server = http.createServer((요청, 응답) => { 

    const _url = 요청.url;
    const url_list = _url.split('/')
    const _method = 요청.method;
    const _ip = 요청.headers['x-forwarded-for'] ||  요청.connection.remoteAddress
    console.log(_ip, decodeURIComponent(_url),_method); //'[level]',level, 
    
    Login.server(요청, 응답, (요청, 응답, level)=>{
        if (_url == '/' && _method == "GET") {
            응답.writeHead(200, {'Content-Type': 'text/html; charset=utf-8' });
            응답.end(make_index_html(file_list_2_html(),level));
        }
            ...
            url_list[1]=='files' && 
            ok(decodeURIComponent(url_list[2])) && 
            fs.existsSync('./files/'+decodeURIComponent(url_list[2]))
            ){
